...
let PermissionsGuard = class PermissionsGuard {
    ...
    async canActivate(context) {
        ...
        if (!dbUser) {
            throw new common_1.ForbiddenException('Yêu cầu bị từ chối: Tài khoản không tồn tại hoặc đã bị xoá');
        }
        const userPermissions = new Set(dbUser.userRoles.flatMap((ur) => ur.role.rolePermissions.map((rp) => rp.permission.name)));
        const hasAllPermissions = requiredPermissions.every((permission) => userPermissions.has(permission));
        if (!hasAllPermissions) {
            throw new common_1.ForbiddenException('Yêu cầu bị từ chối: Bạn không có quyền thực hiện hành động này');
        }
        return true;
    }
};
